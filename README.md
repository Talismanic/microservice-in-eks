...
Now you need to copy the reverse proxy EXTERNAL_IP and change the configuration of the [front end prod](https://github.com/Talismanic/udagram-microservice/blob/master/udacity-c3-frontend/src/environments/environment.prod.ts) & [frond end ](https://github.com/Talismanic/udagram-microservice/blob/master/udacity-c3-frontend/src/environments/environment.ts) to use this IP as backend instead of localhost. Note that, to access the feed and user service from frontend service from browser, you need to add CORS header in the [nginx.conf](https://github.com/Talismanic/udagram-microservice/blob/master/udacity-c3-deployment/docker/nginx.conf) file. Details changes are inside the repo. Now rebuild the docker image for frontend service and reverse proxy following step 01. Then update the kubernetes deployments applying kubectl apply command similar to step 03.
